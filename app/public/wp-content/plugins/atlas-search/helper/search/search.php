...
class Search {
	...
	public function search_content_engine( string $search_query, array $order_by, int $limit = 10, int $offset = 0, array $search_fields = null, bool $fuzzy = true, int $fuzzy_distance = 1, ?string $filter = null ) {
		$graphql_query = <<<'GRAPHQL'
			query Search($query: String!, $limit: Int, $offset: Int, $fields: [String], $fuzzy: Boolean, $fuzzyDistance: Int, $orderBy: [SearchOrderBy!], $filter: String) {
				search(query: $query, limit: $limit, offset: $offset, fields: $fields, fuzzy: $fuzzy, fuzzyDistance: $fuzzyDistance, orderBy: $orderBy, filter: $filter) {
					result
				}
			}
		GRAPHQL;
		...
		return array( 'result' => $response['data']['search']['result'] ?? array() );
	}
	...
	public function get_ce_posts( ?array $posts, WP_Query $query ) {
		...
		$search_fields = $fuzzy_config['enabled'] ? $this->get_fuzzy_search_fields( $post_types, $config['models'] ) : $this->get_default_search_fields( $post_types, $config['models'] );
		$order_by      = array( array( 'published_at' => 'desc' ) );
		...
		// Pick out the hits.
		$hits = $ce_results['result']['hits']['hits'];

		// setup for found documents.
		$found_documents = $ce_results['result']['hits']['total']['value'];
	...
	public function generate_filters( array $post_types ): ?string {
		...
		return "AND (post_type:{$types_str} OR postType:{$types_str})";
	}

	public function get_fuzzy_search_fields( array $post_types, array $models_config ): array {
		$fields = array();
		...
		foreach ( $post_types as $post_type ) {
			...
			foreach ( $config_for_post_type as $key => $value ) {
				$weight = $value['weight'];
				if ( true === $value['searchable'] && ! in_array( "$key^$weight", $fields, true ) ) {
					$fields[] = "$key^$weight";
				}
			}
		}
	...
	public function get_default_search_fields( array $post_types, array $models_config ): array {
		$fields = array();
	...
	protected function format_hits_as_ids( $hits ): array {
		...
		foreach ( $hits as &$hit ) {
			$source = $hit['_source'];
			if ( isset( $source['wpId'] ) ) {
				array_push( $result_posts, $source['wpId'] );
			} else {
				array_push( $result_posts, $source['id'] );
			}
		}
