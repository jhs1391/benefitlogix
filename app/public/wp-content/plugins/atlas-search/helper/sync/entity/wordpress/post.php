...
class Post extends WP_Entity {
	...
	public function upsert( int $post_id, WP_Post $post, ?Acf $acf_info = null ) {
		...
		$wp_featured_image_url = null;

		if ( has_post_thumbnail( $post_id ) ) {
			$wp_featured_image_id = get_post_thumbnail_id( $post_id );
			$featured_image       = WP_Post::get_instance( $wp_featured_image_id );
			if ( ( $featured_image instanceof WP_Post ) && Post_Type::ATTACHMENT === $featured_image->post_type ) {
				$wp_featured_image_url = $featured_image->guid;
			}
		}
		...
		$graphql_vars = array(
			...
			'wpFeaturedImageId'  => $wp_featured_image_id,
			'wpFeaturedImageUrl' => $wp_featured_image_url,
